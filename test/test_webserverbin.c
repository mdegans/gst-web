...
// it's easier to *test* GStreamer in C because many macros related
// to testing are not bound by .vapi (they can be but this seems to not be the practice)
// this also has the benefit of making sure the headers are generated properly

#include <gst/check/check.h>

#include "gstweb.h"

static const char* ELEMENT_NAME = "webserverbin";
static const char* ELEMENT_TYPE_NAME = "GstWebServerBin";
...
//     GST_PAD_ALWAYS,
//     GST_STATIC_CAPS(
//         GST_VIDEO_CAPS_MAKE_WITH_FEATURES(GST_WEB_SERVER_BIN_SINK_CAPS_STR)));
...
GST_START_TEST(test_type) {
  ...
  ck_assert(GST_WEB_IS_SERVER_BIN(filter));
...
GST_START_TEST(test_autoptr) {
  ...
  g_autoptr(GstElement) filter = gst_element_factory_make(ELEMENT_NAME, NULL);
}
GST_END_TEST;
...
//   gst_check_setup_events(sink_pad, filter, caps, GST_FORMAT_TIME);
